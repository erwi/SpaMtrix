...
// #include "mvvrf.h" // DECLARES STRUCT "MV_Vector_" USED AS INDICATOR OF REFERENCE/NO REFERENCE. USE BOOL INSTEAD
...
{                                                                      
    ...
    public:                                                            
     ...
     // Eero's changes
    void point_to(int* p, int len);	// THIS DOES NOT *OWN* THE DATA, AND DOES NOT DEALLOCATE WHEN DECONSTRUCTED
    ...
    //
    MV_Vector_int(int* d,  int N, bool ref) :
                            p_(d), dim_(N), ref_(ref) {}

    MV_Vector_int(const MV_Vector_int &V, bool ref)   :
                            p_(V.p_), dim_(V.dim_), ref_(ref) {}
